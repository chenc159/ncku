...
ang2 = 90*math.pi/180
show_animation = True
def calc_4points_bezier_path(sx, sy, syaw, ex, ey, eyaw, offset, wpt_num): #給定控制點輸入 控制點起點(現在的位置) 終點(下一秒的位置) 起始角度(自己) 結速角度(長機)
    """
    ...
    path = calc_bezier_path(control_points, n_points=wpt_num) # path存放了[0,1,sample = 5 ]每一個位置點 = traj[ x,y
                                                                                                     #x1,y1 ..]
